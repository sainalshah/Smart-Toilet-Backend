...
module.exports = function(passport) {
  ...
  passport.deserializeUser(function(user, done) {
    ...
    if(user.role_id==1){
      ...
      connection.query("SELECT * FROM Doctor WHERE doctor_id = ? ",[user.doctor_id], function(err, rows){
        if(err){
          throw err;
        }
  ...
  function insertNewDoctor(newUserMysql,done) {
    ...
    var insertQuery = `INSERT INTO Doctor ( doctor_email, password,doctor_name,
    clinic_id ) values (?,?,?,?)`;
    connection.query(insertQuery,[newUserMysql.username, newUserMysql.password,
      newUserMysql.name,newUserMysql.clinic_id],
      function(err, rows) {
        if(err){
          throw err;
        }
        newUserMysql.doctor_id = rows.insertId;
    ...
    function insertNewUser(newUserMysql,done) {
      ...
      connection.query(insertQuery,[newUserMysql.username, newUserMysql.password,
        newUserMysql.phone_number,newUserMysql.name,newUserMysql.postal,newUserMysql.coordinates],
        function(err, rows) {
          if(err){
            throw err;
          }
    ...
    passport.use(
      ...
      function(req, email, password, done) {
        ...
        connection.query("SELECT * FROM Doctor WHERE doctor_email = ?",[email], function(err, rows) {
          if (err)
          return done(err);
          if (rows.length) {
    ...
    passport.use(
      ...
      function(req, username, password, done) { // callback with email and password from our form
        ...
        connection.query("SELECT * FROM Doctor WHERE doctor_email = ?",[username], function(err, rows){
      ...
      function(req, username, password, done) {
        ...
        connection.query("SELECT * FROM User WHERE user_email = ?",[username], function(err, rows) {
          ...
          if (rows.length) {
            ...
            url = `http://maps.googleapis.com/maps/api/geocode/json?address=`+req.body.postal+`&sensor=true`;
            getLocation(url,function(err, body) {
              if(err){
                console.error("Unable to request distance");
                throw err;
              }
